...
$VERSION = "3";
%IRSSI = (
    authors     => "Timo Sirainen, Ian Peters, David Leadbeater, Dave B.",
    contact		=> "tss\@iki.fi", 
    name        => "Nick Color",
  ...
  if (!$op) {
    ...
    if (!$nick) {
      ...
      Irssi::print ("Color not given");
        ...
        my @list;
        foreach my $item (keys %colors) {
            push @list, $colors{$item} . $item . '%N';
        }
      Irssi::print ("Invalid color. Valid colors are: " . join ', ', @list);
    } else {
